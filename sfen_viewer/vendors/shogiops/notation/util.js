...
export function aimingAt(pos, pieces, to) {
    let ambs = SquareSet.empty();
    for (const p of pieces)
        if (pos.illegalMoveDests(p).has(to))
            ambs = ambs.with(p);
    return ambs;
}
export function roleToWestern(rules) {
    if (rules === 'dobutsu')
        return doubutsuRoleToWestern;
    return (role) => {
        switch (role) {
            case 'pawn':
                return 'P';
...
function doubutsuRoleToWestern(role) {
    switch (role) {
        case 'pawn':
            return 'C';
        case 'bishop':
...
export function roleKanjiDuplicates(rules) {
    if (rules === 'chushogi')
        return (role) => {
            const roles = [
                ['gold', 'promotedpawn'],
...
export function roleToKanji(rules) {
    if (rules === 'dobutsu')
        return doubutsuRoleToKanji;
    return (role) => {
        switch (role) {
            case 'pawn':
                return '歩';
...
function doubutsuRoleToKanji(role) {
    switch (role) {
        case 'pawn':
            return 'ひよこ';
        case 'bishop':
...
export function roleToBoardKanji(rules) {
    if (rules === 'dobutsu')
        return doubutsuRoleToBoardKanji;
    return (role) => {
        switch (role) {
            case 'promotedlance':
                return '杏';
...
function doubutsuRoleToBoardKanji(role) {
    switch (role) {
        case 'pawn':
            return 'ひ';
        case 'bishop':
...
export function roleToFullKanji(rules) {
    if (rules === 'dobutsu')
        return doubutsuRoleToKanji;
    return (role) => {
        switch (role) {
...
export function filesByRules(rules) {
    switch (rules) {
        case 'chushogi':
...
export function pieceToBoardKanji(rules) {
    return (piece) => {
        if (piece.color === 'gote')
            return `v${roleToBoardKanji(rules)(piece.role)}`;
        else
            return roleToBoardKanji(rules)(piece.role);
    };
}
...
export function makeJapaneseSquareHalf(sq) {
...
export function parseJapaneseSquare(str) {
    ...
    const file = Number.parseInt(str
        .slice(0, fileOffset)
        .split('')
    ...
    if (Number.isNaN(file) || file < 0 || file >= 16 || rank < 0 || rank >= 16)
        return;
    return file + 16 * rank;
}
